...
gint flag, gint memo_line) {
	...
	if(extra_data->type == RLIB_ELEMENT_IMAGE) {
		...
		if(extra_data->delayed == TRUE) {
			...
			gchar *real_text = rlib_layout_get_true_text_from_extra_data(r, extra_data, memo_line, spaced_out);
